...
import { ZERO_ADDRESS } from '../swapConstants';
...
describe('computeUniCirculation', () => {
  ...
  it('before staking', () => {
    expect(computeUniCirculation(token, 0n, undefined)).toEqual(tokenAmount(150_000_000));
    expect(computeUniCirculation(token, 1600387200n, undefined)).toEqual(tokenAmount(150_000_000));
  });
  it('mid staking', () => {
    expect(computeUniCirculation(token, BigInt(1600387200 + 15 * 24 * 60 * 60), undefined)).toEqual(
      tokenAmount(155_000_000)
    );
  });
  it('after staking and treasury vesting cliff', () => {
    expect(computeUniCirculation(token, BigInt(1600387200 + 60 * 24 * 60 * 60), undefined)).toEqual(
      tokenAmount(224_575_341)
    );
  });
  it('subtracts unclaimed uni', () => {
    expect(computeUniCirculation(token, BigInt(1600387200 + 15 * 24 * 60 * 60), tokenAmount(1000))).toEqual(
